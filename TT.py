...
class TT(QMainWindow, Ui_MainWindow):
	...
	def manageTyping(self):
		...
		SpeedAppender = self.LetCounter / ((time.time() - self.timerStart - self.ST) / 60)
		self.Speed.append(int(SpeedAppender))
		...
		try:
			...
			if self.start:
				...
				if self.typed == '': # If the session has started and the first butt
						     # on is being pressed
					self.timerStart = time.time() # start position for the timer
								      # is set
				...
				if self.TextField.text() == '' and self.TextTyped.text() != '':
					self.offKey() # When there is nothing more to type all the
						      # keys turn off
					self.finishing()
	...
	def finishing(self):
		if not self.finish: # If the session is not finished
			...
			lCount = self.LetCounter / ((time.time() - self.timerStart - self.ST) / 60)
			self.LetCounter = int(lCount)
			if self.LetCounter <= 425 and self.LetCounter > 300:
				self.Time.setStyleSheet("color: rgb(44, 56, 97)")
			elif self.LetCounter >= 425:
				self.Time.setStyleSheet("color: rgb(1, 255, 1)")
			else:
				self.Time.setStyleSheet("color: rgb(255, 138, 84)")
			self.Time.setText(str(self.LetCounter) + ' characters per minute')
			self.Time.show()
			...
			writer = ' '.join([self.Modes[self.Mode] + ',',
					  str(self.LetCounter), 'cpm,', str(self.errors), '% errors',
